...
export default function OrganizationExplorerScreen() {
  ...
  const fetchRows = useCallback(async (nextPage = 1, append = false) => {
    ...
    try {
      const response = await api.getOrganizations({ ...filters, page: nextPage, limit: 50 })
      if (response.success) {
        ...
        setPage(nextPage)
        setOffline(false)
        if (tablet && !append && nextRows.length > 0) setSelectedId((current) => current ?? nextRows[0].id)
      }
    } catch (error: any) {
      if (error.message !== "SESSION_EXPIRED" && agent) {
        try {
          const cached = await readOfflineOrganizations(agent.organizationId, agent.id, filters.search ?? "")
  ...
  const openRow = (item: ExplorerOrganization) => {
    if (selectedIds.size > 0) return toggleSelected(item.id)
  ...
  const renderRow = ({ item }: { item: ExplorerOrganization }) => {
    ...
    return (
      <Pressable
        ...
        onLongPress={() => canManage && toggleSelected(item.id)}
        style={[styles.card, active && styles.cardActive, checked && styles.cardChecked]}
      >
        <View style={styles.cardTop}>
          {selectedIds.size > 0 ? <View style={[styles.checkbox, checked && styles.checkboxOn]}><Text style={styles.checkboxText}>{checked ? "✓" : ""}</Text></View> : null}
  ...
  const list = (
    <View style={[styles.listPane, tablet && styles.tabletListPane]}>
      <View style={styles.toolbar}>
        ...
        <TouchableOpacity style={[styles.iconButton, filterCount > 0 && styles.iconButtonActive]} onPress={() => setFilterOpen(true)}>
          <Text style={[styles.iconButtonText, filterCount > 0 && styles.iconButtonTextActive]}>⚙ {filterCount || ""}</Text>
        </TouchableOpacity>
        {canManage ? <TouchableOpacity style={styles.iconButton} onPress={() => setColumnsOpen(true)}><Text style={styles.iconButtonText}>☷</Text></TouchableOpacity> : null}
      </View>

      {canManage && views.length > 0 ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.viewBar}>
          {views.map((view) => <Pressable key={view.id} style={styles.viewChip} onPress={() => applyView(view)}><Text style={styles.viewChipText}>{view.name}</Text></Pressable>)}
          <Pressable style={styles.viewAdd} onPress={() => setSaveOpen(true)}><Text style={styles.viewAddText}>＋</Text></Pressable>
        </ScrollView>
      ) : canManage ? (
        <Pressable style={styles.saveHint} onPress={() => setSaveOpen(true)}><Text style={styles.saveHintText}>{t("organizations.saveView")}</Text></Pressable>
      ) : null}
      ...
      <View style={styles.resultHeader}>
        ...
        {canManage && selectedIds.size === 0 ? <Text style={styles.selectHint}>{t("organizations.longPressSelect")}</Text> : null}
      </View>

      {selectedIds.size > 0 ? (
        <View style={styles.selectionBar}>
          <Text style={styles.selectionCount}>{t("organizations.selectedTemplate", { n: selectedIds.size })}</Text>
          <Pressable style={styles.selectionAction} onPress={() => openAssignment("ASSIGN")}><Text style={styles.selectionActionText}>{t("organizations.assign")}</Text></Pressable>
          <Pressable style={styles.selectionActionSecondary} onPress={() => openAssignment("UNASSIGN")}><Text style={styles.selectionActionSecondaryText}>{t("organizations.unassign")}</Text></Pressable>
          <Pressable onPress={() => setSelectedIds(new Set())}><Text style={styles.clearSelection}>×</Text></Pressable>
        </View>
      ) : null}
...
function FilterSheet({ visible, tablet, filters, facets, onChange, onClose, t }: any) {
  ...
  return <Sheet visible={visible} onClose={onClose} tablet={tablet}><SheetHeader title={t("organizations.filters")} onClose={onClose} />
    <ScrollView contentContainerStyle={styles.sheetScroll}>
      <FilterRow label={t("organizations.assignmentState")} values={["ASSIGNED", "UNASSIGNED"]} selected={filters.assignmentState} onSelect={(value) => set("assignmentState", value)} />
      <FilterRow label={t("organizations.objectType")} values={["PHARMACY", "CLINIC", "STORE", "OTHER"]} selected={filters.objectType} onSelect={(value) => set("objectType", value)} />
      <FilterRow label={t("organizations.category")} values={["A", "B", "C", "D"]} selected={filters.category} onSelect={(value) => set("category", value)} />
      <FilterRow label={t("organizations.status")} values={["ACTIVE", "INACTIVE", "PROSPECT"]} selected={filters.status} onSelect={(value) => set("status", value)} />
...
function FilterRow({ label, values, labels = {}, selected, onSelect }: { label: string; values: string[]; labels?: Record<string, string>; selected?: string; onSelect: (value: string) => void }) {
  ...
  return <View style={styles.filterRow}><Text style={styles.filterLabel}>{label}</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pills}>{values.map((value) => <Pressable key={value} style={[styles.pill, selected === value && styles.pillOn]} onPress={() => onSelect(value)}><Text style={[styles.pillText, selected === value && styles.pillTextOn]}>{labels[value] ?? value}</Text></Pressable>)}</ScrollView></View>
}
...
function ColumnsSheet({ visible, columns, onChange, onClose, t }: any) {
  ...
  return <Sheet visible={visible} onClose={onClose}><SheetHeader title={t("organizations.columns")} onClose={onClose} /><View style={styles.columnGrid}>{ORGANIZATION_COLUMNS.map((column) => <Pressable key={column} style={[styles.columnChoice, columns.includes(column) && styles.columnChoiceOn]} onPress={() => toggle(column)}><Text style={[styles.columnChoiceText, columns.includes(column) && styles.columnChoiceTextOn]}>{columns.includes(column) ? "✓ " : ""}{t(`organizations.column_${column}`)}</Text></Pressable>)}</View><View style={styles.sheetFooter}><Pressable style={styles.primaryButton} onPress={onClose}><Text style={styles.primaryButtonText}>{t("common.done")}</Text></Pressable></View></Sheet>
}
...
function SheetHeader({ title, onClose }: { title: string; onClose: () => void }) { return <View style={styles.sheetHeader}><View><Text style={styles.sheetEyebrow}>LEADDRIVE MTM</Text><Text style={styles.sheetTitle}>{title}</Text></View><Pressable style={styles.sheetClose} onPress={onClose}><Text style={styles.sheetCloseText}>×</Text></Pressable></View> }
...
const styles = StyleSheet.create({
  ...
  viewBar: { paddingHorizontal: 14, gap: 8, paddingBottom: 8 }, viewChip: { backgroundColor: "#fff", paddingHorizontal: 12, paddingVertical: 7, borderRadius: 999, borderWidth: 1, borderColor: "#cbe1dd" }, viewChipText: { color: "#285a56", fontSize: 12, fontWeight: "700" }, viewAdd: { width: 32, height: 32, alignItems: "center", justifyContent: "center", borderRadius: 16, backgroundColor: "#0f766e" }, viewAddText: { color: "#fff", fontSize: 18 }, saveHint: { marginHorizontal: 14, marginBottom: 8, alignSelf: "flex-start" }, saveHintText: { color: "#0f766e", fontSize: 12, fontWeight: "700" },
  offline: { marginHorizontal: 14, padding: 8, borderRadius: 10, backgroundColor: "#fff7ed" }, offlineText: { color: "#b45309", fontSize: 12, fontWeight: "700" }, resultHeader: { flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 7 }, resultCount: { color: "#536b69", fontSize: 12, fontWeight: "700" }, selectHint: { color: "#8aa09e", fontSize: 11 },
  selectionBar: { marginHorizontal: 12, marginBottom: 8, padding: 8, borderRadius: 14, backgroundColor: "#123c38", flexDirection: "row", alignItems: "center", gap: 7 }, selectionCount: { color: "#fff", fontSize: 12, fontWeight: "800", flex: 1 }, selectionAction: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 9, backgroundColor: "#38b2a4" }, selectionActionText: { color: "#fff", fontSize: 11, fontWeight: "800" }, selectionActionSecondary: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 9, backgroundColor: "#fff" }, selectionActionSecondaryText: { color: "#123c38", fontSize: 11, fontWeight: "800" }, clearSelection: { color: "#fff", fontSize: 22, paddingHorizontal: 4 },
  ...
  backdrop: { flex: 1, backgroundColor: "rgba(5,24,22,0.42)", justifyContent: "flex-end" }, sheet: { maxHeight: "88%", backgroundColor: "#fff", borderTopLeftRadius: 26, borderTopRightRadius: 26, overflow: "hidden" }, sheetTablet: { width: 520, maxHeight: "92%", alignSelf: "flex-end", borderTopLeftRadius: 26, borderTopRightRadius: 0 }, sheetHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 20, borderBottomWidth: 1, borderBottomColor: "#e6eeed" }, sheetEyebrow: { color: "#38a89c", fontSize: 10, fontWeight: "900", letterSpacing: 1.2 }, sheetTitle: { color: "#102a2a", fontSize: 20, fontWeight: "900", marginTop: 3 }, sheetClose: { width: 38, height: 38, borderRadius: 19, backgroundColor: "#edf5f3", alignItems: "center", justifyContent: "center" }, sheetCloseText: { fontSize: 25, color: "#536b69", marginTop: -2 }, sheetScroll: { padding: 18 }, sheetFooter: { flexDirection: "row", justifyContent: "flex-end", gap: 10, padding: 16, borderTopWidth: 1, borderTopColor: "#e6eeed" },
  filterRow: { marginBottom: 17 }, filterLabel: { color: "#365c58", fontSize: 12, fontWeight: "800", marginBottom: 8 }, pills: { gap: 7 }, pill: { borderRadius: 999, borderWidth: 1, borderColor: "#cddfdd", paddingHorizontal: 12, paddingVertical: 8, backgroundColor: "#f9fbfb" }, pillOn: { backgroundColor: "#0f766e", borderColor: "#0f766e" }, pillText: { color: "#496d69", fontSize: 12, fontWeight: "700" }, pillTextOn: { color: "#fff" },
  primaryButton: { minWidth: 130, minHeight: 46, paddingHorizontal: 18, borderRadius: 13, backgroundColor: "#0f766e", alignItems: "center", justifyContent: "center" }, primaryButtonText: { color: "#fff", fontSize: 13, fontWeight: "900" }, secondaryButton: { minWidth: 100, minHeight: 46, paddingHorizontal: 18, borderRadius: 13, backgroundColor: "#edf5f3", alignItems: "center", justifyContent: "center" }, secondaryButtonText: { color: "#245a55", fontSize: 13, fontWeight: "800" }, buttonDisabled: { opacity: 0.45 }, formBody: { padding: 20 }, inputLabel: { color: "#365c58", fontSize: 12, fontWeight: "800", marginBottom: 7, marginTop: 8 }, input: { minHeight: 46, borderWidth: 1, borderColor: "#cddfdd", borderRadius: 12, paddingHorizontal: 13, color: "#102a2a", backgroundColor: "#fbfdfd" }, reasonInput: { minHeight: 88, textAlignVertical: "top", paddingTop: 12 },
  columnGrid: { padding: 18, flexDirection: "row", flexWrap: "wrap", gap: 9 }, columnChoice: { width: "48%", padding: 13, borderRadius: 12, borderWidth: 1, borderColor: "#dce7e5", backgroundColor: "#f8fbfa" }, columnChoiceOn: { borderColor: "#0f766e", backgroundColor: "#e8f6f3" }, columnChoiceText: { color: "#5c7471", fontSize: 12, fontWeight: "700" }, columnChoiceTextOn: { color: "#0f766e" },
