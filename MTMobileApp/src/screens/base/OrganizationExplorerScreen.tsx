  ...
  const list = (
    <View style={[styles.listPane, tablet && styles.tabletListPane]}>
      ...
      <View style={styles.resultHeader}>
        <Text style={styles.resultCount}>{t("organizations.totalTemplate", { count: total })}</Text>
      ...
      {selectedIds.size > 0 ? (
        <View style={styles.selectionBar}>
          <View style={styles.selectionCopy}>
            <Text style={styles.selectionCount}>{t("organizations.selectedTemplate", { count: selectedIds.size })}</Text>
            {hiddenSelectedCount > 0 ? <Text style={styles.hiddenSelectionWarning}>{t("organizations.hiddenSelectedTemplate", { count: hiddenSelectedCount })}</Text> : null}
          </View>
...
function ColumnRows({ item, columns, t }: { item: ExplorerOrganization; columns: OrganizationColumn[]; t: (key: string, options?: any) => string }) {
  const rows: Array<{ key: OrganizationColumn; icon: string; value?: string }> = [
    ...
    { key: "contacts", icon: "♙", value: item.contactsCount != null ? t("organizations.contactsTemplate", { count: item.contactsCount }) : undefined },
    { key: "visits", icon: "✓", value: item.visitsCount != null ? t("organizations.visitsTemplate", { count: item.visitsCount }) : undefined },
  ]
