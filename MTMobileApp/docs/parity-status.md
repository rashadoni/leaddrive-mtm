...
Быстрая справка «что частично / что отсутствует» по 18 экранам эталонной CRM.
Источник истины и полные критерии «есть/нет» — `reference-parity-register.md` (v0.4).
...
- **SWM-07 Мои организации** — ✅ фильтры эталонной CRM, saved views, phone cards,
...
- **SWM-10 GPS history — Partial** — ✅ фоновый GPS, heartbeat, серверная проверка границ рабочего дня, `getLocationHistory()`, Agent self-history, дата в timezone тенанта, локальная схема, playback и честные GPS-gap/invalid-point notices · ❌ Manager employee/time filters, серверные stops/visits/anomalies, durable offline history, retention/export и physical-device lifecycle proof.
- **SWM-11 Replay маршрута — Partial** — ✅ плановый маршрут отдельно, фактическая GPS-линия, серверная привязка GPS к рабочему дню и дневной raw-point replay с разрывами · ❌ единый plan-vs-fact, серверные stops/visits и proven durable offline replay.
- **SWM-12 Live team map — Partial** — ✅ серверно scoped команда, принятые координаты, current/last-known/stale маркеры, timestamp/accuracy/online, локальная privacy-safe схема, fit/zoom и 60-секундное обновление · ❌ поиск/фильтры, кластеризация, current visit/route drill-down, push real-time и physical-device proof.
