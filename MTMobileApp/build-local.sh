...
# mtm-release.keystore, поэтому APK подписывается релизным ключом и
# обновляет уже установленное приложение (CI без секрета подписывает
# debug-ключом — только через удаление/переустановку).
#
...
if [ ! -f android/app/mtm-release.keystore ]; then
  ...
  echo "   (поверх установленного релизного приложения не встанет)"
fi
