...
Коротко: в `main` только через PR; обязательные проверки — `scope` и `tests`
из `ci.yml` (jest и tsc; платного ИИ-ревьюера нет, `agent-review` снят
2026-09-14); APK собирается в CI с fail-closed подписью; перед мержем того, что
